...
def plot_harmonic_1_6_full(poms_model, Is, t, output_filename, mod=1):
    ...
    f,((ax1,ax2),(ax3,ax4),(ax5,ax6))= plt.subplots(3,2,figsize= (10,8.4))
    ...
    for i in range(len(ia)):
        for n,I in zip(range(len(Is)),Is):
            ...
            if cfreq > 0:
                F = np.concatenate((F_filt[cfreq:],np.zeros(len(F)-2,dtype=complex),F_filt[0:cfreq]))
            else:
            ...
            if len(I)%2 == 0:
                ax.plot(t[0::mod]*T_0,F[0::mod]*I_0*1e6,label=label,c=colors[n],ls='-')
            else:
                ax.plot(T_0*t[0::mod],F[0::mod]*I_0*1e6,label=label,c=colors[n],ls='-')
